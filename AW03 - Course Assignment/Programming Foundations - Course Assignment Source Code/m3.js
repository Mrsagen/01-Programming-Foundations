...
function storeValue() {
    ...
    if(result !== null) {
        alert("data provided: " + result);
        global = result;
    } 
    
    else {

        alert("current data: " + global);
    }
}
...
function extractMiddleString() {
    ...
    const length = Math.ceil(text.length / 2);
...
function stringReplace() {
    ...
    const replacedText = text.replace(/a/gi, '@').replace(/e/gi, '3').replace(/o/gi, '0'); 
    alert(replacedText);
}
